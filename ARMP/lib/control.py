...
def make_case(dataclass, combi, dic):
    ...
    season_month = case_across_list(layout_dic['season'], dic['season_list'], dic['season_month_list'])
    ...
    case_keys = [field.name for field in fields(dataclass)] # get defined keys in dataclass

    dic_list_keys = modify_list_keys(dic) # return keys endwith _list, removing _list
    dic_list_keys = [item for item in dic_list_keys if item not in list(layout_dic.keys()) ]
    ...
    if isinstance(dataclass, Case):
        ...
        fn_in = "{}_tag_list.txt".format('_'.join((layout_dic['model'], layout_dic['ARDT'])))
