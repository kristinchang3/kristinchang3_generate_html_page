...
def metric_plot_character_panel(dict_in, model_list, ARDT_list, region_list, season_list, field_list,
                                axis_labels, yaxis_labels, cbar_label,
    ...
    for i, region in enumerate(region_list[:]):
        ...
        ax[i].pcolor(x, y, np.where((zscore >= -1.96) & (zscore < 1.96), matrix, np.nan), ec=None, lw=0.01, hatch='/////', alpha=0)
    ...
    cbar = fig.colorbar(im, cax=cbar_ax, orientation='vertical', fraction=0.017, pad=10.0)
...
if __name__ == "__main__":
    ...
    metric_plot_character_panel(dict_in, model_list, ARDT_list, region_list, season_list, field_list,
                                axis_labels, yaxis_labels, cbar_label,
                                title, fig_filename, fig_dir)
