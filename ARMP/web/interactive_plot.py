...
import os, os.path
from bokeh.models import HoverTool, LogTicker, ColorBar, LinearColorMapper, LogColorMapper, Label
from bokeh.models import HTMLLabel, Rect, Grid, ColumnDataSource
...
if __name__ == "__main__":
    ...
    for i, model in enumerate(model_list):
        for j, region in enumerate(region_list):
            filename = img_path+"map_freq_"+model+"_"+str_fn(region)+"_"+ARDT+"_"+season+".png"
    ...
    def adjust_clabel(plot, element):
        ...
        label = HTMLLabel(x=100*num_regions + 80,
    ...
    # metric plot
    spatial_corr = dd.hvplot.heatmap(y='model',
                           x='region',
                           C='correlation',
    ...
    spatial_corr = spatial_corr * hv.Labels(spatial_corr).opts(text_color='correlation',
                                                                        color_levels=bins,
                                                                        cmap=label_colors,
                                                                        xaxis='top'
                                                                        )
