...
def map_frequency(dic, mp, mp_ref, model, model_ref, ARDT, region, season):
    ...
    ax1.set_extent(
        [np.min(mp.lon), np.max(mp.lon), np.min(mp.lat), np.max(mp.lat)], projection
    )
    ...
    fig_filename = (
        "map_freq_" + model + "_" + str_fn(region) + "_" + ARDT + "_" + season
    )
    plt.savefig(
        os.path.join(dic["dir_fig"], fig_filename) + ".png",
        bbox_inches="tight",
        dpi=300,
    )
...
def plot_map_frequency(dic):
    ...
    for combi in product(*layout_pool):
        ...
        map_frequency(
            dic, freq_reg_mp, freq_reg_mp_ref, model, model_ref, ARDT, region, season
        )
